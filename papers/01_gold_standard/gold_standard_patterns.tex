...
1{,}509 GBFS Systems, Seven Anomaly Classes and One Operator-Specific Hotspot per Country}
...
\textbf{taxonomy} of seven anomaly classes (A1--A5 plus the two
candidate classes A6 and A7 discovered through the global extension),
derived from the audit of all 123 French GBFS systems and stress-tested
on three non-French systems (Bicing Barcelona, Oslo Bysykkel, Bergen
Bysykkel) which all pass the audit with zero anomalies under unchanged
thresholds. Second, a
...
\begin{table}[ht]
\centering
\caption{Seven GBFS anomaly classes observed on the French (FR) and
the global non-French (Global) corpora. A1--A5 are part of the
original taxonomy; A6 and A7 were discovered during the global audit
and are formalised in Sections~\ref{secA6Formalisation}
and~\ref{secA7Formalisation}. Incidence is at the system level except
where noted.}
\label{tab:anomalies}
\small
\begin{tabular}{@{}lllrr@{}}
...
of its entries. Unlike A6, where $\tau$ was anchored to the empirical
distribution percentiles, A7 uses a coarser threshold because
$\rho_s^{A7}$ is extremely bimodal on the global corpus
(65\,\% of audited systems are at exactly $0\,\%$ NaN and 28\,\% at
exactly $100\,\%$ NaN, with very few intermediate cases). Any
threshold in $(0, 1)$ yields essentially the same flagged set;
$\tau_{A7} = 0.5$ is reported for parsimony and is robust to
sensitivity sweeps in the tested range.

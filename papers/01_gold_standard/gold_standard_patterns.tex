...
the same rule set to the entire MobilityData canonical catalogue
(1{,}509 GBFS systems across 48 countries, of which 1{,}421
reachable and 917 with published station data) and observe that
204 systems trigger at least one A1--A5 class. Two candidate
classes emerge from the global audit: A6 (zero-capacity dock)
flags 14 systems, and A7 (null capacity field) catches an
additional 215 systems and 70{,}176 stations that the v1.0 taxonomy
A1--A5 was structurally blind to. The audit therefore covers the
entire registered world catalogue, and a side finding is that the
French national portal \emph{transport.data.gouv.fr} indexes only
about half of the 255 French entries that MobilityData lists. The Czech
...
trust. By auditing the entire MobilityData canonical catalogue of 1{,}509
GBFS systems across 48 countries, we document that the same five
anomaly classes recur worldwide, and that hotspots are typically
driven by a single operator (nextbike in Czech Republic, Pony in
France) rather than by national policy. The contribution is methodological as much as
...
\begin{highlights}
\item Exhaustive audit of all 1{,}509 GBFS systems in the world catalogue
\item Operator-driven hotspots: nextbike in Czechia, Pony in France
\item Two candidate classes (A6, A7) close blind spots on the capacity field
...
A7 & Null capacity field (candidate)& $\geq 50\,\%$ of stations declare $c = $ \texttt{NaN} & 19 (FF) & 215 \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{station\_information} document, and \textbf{166 systems
trigger at least one A1--A5 class} after three methodological
corrections that emerged from the audit itself: a country-perimeter
calibration on A4, a substring-matching fix in the A1 detector (the
GBFS form factor \texttt{cargo\_bicycle} contains the string
\texttt{car} and was initially flagged as A1; the fix tightens A1
to exact list membership on the GBFS v3 form factor enum), and a
convex-hull replacement of the bounding-box surface estimator in A5
(the rectangular bbox overestimates the operational area for
elongated networks; Switzerland's national operators had a bbox
area of 99{,}000\,km$^2$ versus a hull area of $\approx
41{,}000$\,km$^2$). The corrected breakdown is shown in
...
\midrule
\textbf{Total (47 countries)} & 1{,}254 & 1{,}167 & 166 & 29 & 34 & 29 & 78 \\
\bottomrule
\end{tabular}
\end{table}
...
a national level. Because A2 requires a constant non-zero value, A3
requires numerical capacities to compute the profile ratio, A4--A5
are geospatial and A6 tests $c = 0$ (not $c = \texttt{NaN}$), the
existing taxonomy is structurally blind to this case. Generalising
the probe across the audit corpus, \textbf{215 reachable systems
with at least 20 stations have $\geq 50\,\%$ NaN capacity and are
flagged by no A1--A6 class}, together covering 70{,}176 stations.
...
case for introducing a candidate seventh class, A7 (null capacity
field), formalised in Section~\ref{secA7Formalisation}.
...
\subsection{A7 (candidate): null capacity field formalisation}\label{secA7Formalisation}
...
of its entries. The threshold is conservative rather than
distribution-derived because $\rho_s^{A7}$ is bimodal on the global
corpus: a large mass of systems at exactly $0\,\%$ NaN and a
secondary mass at exactly $100\,\%$ NaN, with very few intermediate
cases.

\paragraph{Incidence of A7.}
On the 538 globally audited systems with at least 20 stations,

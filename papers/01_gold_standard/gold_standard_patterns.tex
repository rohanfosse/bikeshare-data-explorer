...
\subsection{Operator-level fragmentation of the capacity field}\label{secConsequences}
...
composite-index work (the companion IMD paper \cite{Fosse2026imd});
demand modelling with graph-based neural networks
\cite{Lin2018BSSGraph} whose false-station nodes from A3 are a known
confound; spatial-equity analysis combining station access with INSEE
Filosofi; and multimodal integration with GTFS, since the
hybridisation schema is compatible with
...
The taxonomy A1--A5 was built by a hybrid inductive-deductive
workflow over five iterations between 2025-04 and 2026-01, starting
from the four ISO/IEC 25012 data-quality dimensions (completeness,
consistency, accuracy, currentness) and enriched by reverse
engineering of the French free-floating fleets. A1 and A4 emerged
from an exploratory scan of the 123 systems; A5 from enrichment with
the \emph{transport.data.gouv.fr} catalogue; A2 from a zero-variance
diagnostic on \texttt{capacity}; A3 from the per-system median
capacity of \emph{Pony} matching a conditional-averaging estimator
more closely than a physical-dock count. Two rejected candidates
(``A6-rolling stock noise'', collapsed into A3; ``A7-cross-border
duplicate'', no instance found) ship with the audit report. The
classes A6 and A7 documented in
Sections~\ref{secA6Formalisation}--\ref{secA7Formalisation} are
distinct from the rejected candidates and emerged from the global
extension rather than the French corpus.
...
46{,}307-station certified Parquet file, the rejected-stations
log, the audit report, and the machine-readable descriptors (JSON
Schema, DCAT-AP, Frictionless Data Package, Croissant JSON-LD) are
deposited on Zenodo under concept DOI
\texttt{10.5281/zenodo.20125460} (current release 1.0.0,
2026-05), mirrored on \emph{Recherche Data Gouv}. The 142 candidate
French GBFS feeds inventoried from \emph{transport.data.gouv.fr}
and MobilityData are archived with SHA-256 hashes in the same
deposit; the certified subset of 123 systems is derived

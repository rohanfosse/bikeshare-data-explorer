...
\begin{equation}
\#\{i \,:\, c_i = 0 \,\land\, \mathtt{is\_virtual\_station}(i) \neq \mathtt{true}\}
\;\geq\; \tau_{A6} \cdot |\mathcal{S}_s|,
\end{equation}
with $\tau_{A6} = 0.01$ as a conservative first threshold. On the
1{,}254-system global corpus, 14 systems exceed 1\,\% zero-capacity
declarations: PBSC HQ (Canada, 18.2\,\%), Beryl Greater Manchester
(UK, 10.5\,\%), MonaBike (Monaco, 6.8\,\%), Bicicoru\~na (Spain,
5.9\,\%), MBajk (Slovenia, 4.8\,\%), CyclOcity (Japan, 4.2\,\%),
\`aV\'elo (Canada, 3.5\,\%), Careem BIKE (UAE, 2.7\,\%), Citi Bike
NYC (US, 2.4\,\%), MiBiciTuBici (Argentina, 2.0\,\%) and four others
between 1.1 and 1.8\,\%. On the French corpus, no dock-based system
exceeds the threshold (0/65 systems with $c = 0$). A6 is reported as
candidate because the threshold $\tau_{A6}$, the interaction with
\texttt{is\_virtual\_station} (only required by GBFS v3) and the
operational consequence for downstream consumers all warrant the

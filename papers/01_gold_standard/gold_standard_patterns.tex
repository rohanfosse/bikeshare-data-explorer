...
release the resulting \emph{GBFS France Audit Catalogue} dataset (46{,}307
...
quantifies the cost of skipping the audit. A characterization of the
free-floating subset documents that the single GBFS
\texttt{capacity} field is populated according to six mutually
incompatible semantics across the four major operators, affecting
84.7\,\% of the certified rows. A global audit on the 1{,}254
non-French GBFS systems of the MobilityData canonical catalogue
flags 226 systems (29.2\,\% of those with data) across 47 countries,
demonstrating that the anti-patterns generalise beyond France: A3
appears in Czech Republic (12) and Germany (9), A2 in Czech Republic
(16), A1 in the DACH region (74). We further publish three internal
robustness checks, a reproducibility checklist with container-pinned
...
\begin{highlights}
\item 226 of 1{,}254 non-French GBFS systems flag at least one A1--A5 anomaly
\item One GBFS field carries six incompatible semantics across operators
\item Bordeaux drops from national rank 2 to rank 14 after one A3 correction
\item Audit Catalogue (46{,}307 stations, 97 cities) FAIR-released on Zenodo
\end{highlights}
...
set was then applied to the entire MobilityData canonical
catalogue: 1{,}254 non-French GBFS systems across 47 countries.
1{,}167 (93.1\,\%) were reachable, 773 publish a
\texttt{station\_information} document, and \textbf{226 systems
(29.2\,\% of those with data) trigger at least one A1--A5 class}
(Table~\ref{tab:global-audit}). A3 (structural over-capacity) is
detected on 29 non-French systems concentrated in Czech Republic
(12) and Germany (9); A2 (placeholder capacity) on 33 non-French
systems with Czech Republic again the hotspot (16); A1
(out-of-domain car-sharing labelled as BSS) on roughly 80 non-French
systems concentrated in the DACH region (Germany 58, Switzerland 11,
Austria 5). Operator-level anti-patterns also travel internationally:
Dott publishes \texttt{capacity = NaN} on every non-French deployment
observed (Austria, UAE; 18 systems), and Bird does the same on its
three Canadian deployments. The cross-country audit yields a
candidate sixth anomaly class, A6 (zero-capacity dock), exemplified
by PBSC HQ (Canada, 18.2\,\%), Beryl Greater Manchester (UK,
10.5\,\%), MonaBike (Monaco, 6.8\,\%) and Citi Bike NYC (US,
2.4\,\%). The empirical conclusion is that the documented
anti-patterns are not French-specific but global, and the protocol
generalises to a 1{,}254-system corpus with no recalibration.

\begin{table}[ht]
\centering
\caption{Global audit of A1--A5 on the MobilityData catalogue
(1{,}254 non-French systems). Per-class counts are not disjoint
(one system can trigger several classes). A4 is sensitive to
country-perimeter calibration; only countries with $\geq 30$
audited systems or notable structural anomaly counts are shown.}

...
MobilityData canonical catalogue and observe that 215 systems
across 47 countries trigger at least one anomaly class. The Czech
...
\begin{highlights}
\item Audit of 1{,}377 GBFS systems across 47 countries flags 22+215 systems
\item One operator drives each country: nextbike in Czechia, Pony in France
\item Single GBFS field carries six incompatible semantics across operators
...
\texttt{station\_information} document, and \textbf{215 systems
trigger at least one A1--A5 class} after country-perimeter
calibration (Table~\ref{tab:global-audit}). A3 (structural
over-capacity) is detected on 29 non-French systems concentrated in
Czech Republic (12) and Germany (9); A2 (placeholder capacity) on
30 non-French systems with Czech Republic again the hotspot (16);
A1 (out-of-domain car-sharing labelled as BSS) on 75 non-French
...
\emph{Dott} publishes \texttt{capacity = NaN} on every non-French
deployment observed (Austria, UAE; 18 systems), and \emph{Bird} does
the same on three Canadian deployments. The cross-country audit also
surfaces a candidate sixth anomaly class, A6 (zero-capacity dock),
exemplified by PBSC HQ (Canada, 18.2\,\%), Beryl Greater Manchester
(UK, 10.5\,\%), MonaBike (Monaco, 6.8\,\%) and Citi Bike NYC (US,
2.4\,\%). The empirical conclusion is that the documented
anti-patterns are not French-specific but global, and the protocol
generalises to a 1{,}254-system corpus with no methodological
recalibration.

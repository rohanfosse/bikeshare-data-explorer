% !TEX TS-program = pdflatex
% !TEX encoding = UTF-8 Unicode
% !BIB TS-program = bibtex
%
% Bayesian station-level Soft Mobility Index (IMD) for French
% bike-sharing systems.
% Authors: R. Foss\'e & G. Pallares (CESI LINEACT, 2025--2026).
%
\documentclass[11pt,a4paper]{article}

% --- Encoding & language ------------------------------------------------
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}
\usepackage[margin=2.5cm]{geometry}
\usepackage{microtype}
\usepackage{csquotes}

% --- Mathematics --------------------------------------------------------
\usepackage{amssymb,amsmath,amstext}

% --- Figures, tables, lists --------------------------------------------
\usepackage{graphicx}
\graphicspath{{figures/}}
\usepackage{booktabs}
\usepackage{array}
\usepackage{float}
\usepackage{caption}
\captionsetup{font=small,labelfont=bf,labelsep=period}
\usepackage{enumitem}
\setlist{topsep=2pt,itemsep=2pt,parsep=0pt}
\usepackage{url}

% --- Citations ----------------------------------------------------------
\usepackage[authoryear,round]{natbib}
\bibliographystyle{elsarticle-harv}

% --- Hyperlinks ---------------------------------------------------------
\usepackage{xcolor}
\usepackage[colorlinks=true,
            linkcolor=black, citecolor=black, urlcolor=blue!50!black,
            breaklinks=true]{hyperref}

% --- Author block -------------------------------------------------------
\usepackage{authblk}
\renewcommand\Authfont{\normalsize}
\renewcommand\Affilfont{\small\itshape}
\setlength{\affilsep}{0.6em}

% --- Convenience macros -------------------------------------------------
\usepackage{xspace}
\newcommand{\IMD}{\mathrm{IMD}}
\newcommand{\IES}{\mathrm{IES}}
\DeclareMathOperator*{\argmax}{arg\,max}

% =========================================================================
\title{Real-time monitoring of French cycling-environment quality:
       a Bayesian station-level composite indicator complementing
       the EMP modal-share survey}

\author[1]{Rohan Foss\'e\thanks{Corresponding author. E-mail:
            \texttt{rohan.fosse@gmail.com}.}}
\author[1]{Ga\"el Pallares}
\affil[1]{CESI LINEACT, BikeShare-ICT research programme,
          Montpellier, France}
\date{}

\begin{document}
\maketitle

% =========================================================================
\begin{abstract}
\noindent
French cycling activity is monitored by the INSEE/SDES
\emph{Enqu\^ete Mobilit\'e des Personnes} (EMP), a household
survey conducted every $7$ to $10$ years that publishes bike
modal shares for $44$ urban areas. The last wave dates from
2018--2019; cycling environments and behaviour have evolved
substantially since. We propose the \emph{Soft Mobility Index}
(IMD), a Bayesian station-level composite of multimodality,
cycling-infrastructure continuity, topography and local
population density, computed on the $46\,139$-station audited
Gold Standard inventory of \citet{Fosse2026gold} and updated
continuously from the General Bikeshare Feed Specification.
The IMD is positioned as a complementary \emph{real-time
monitoring layer} that covers all $600+$ French communes with a
GBFS deployment and refreshes whenever the underlying open data
refreshes.

\textbf{Headline result.} In a head-to-head tournament against
five supply-side and four behavioural references on the French
dock-based panel, the IMD wins the strongest correlation on
three of four references: $\rho = +0.56$ against the FUB
perception score, $\rho = +0.75$ against eco-counter daily
flows (against $+0.55$ for the volumetric default and $+0.71$
for the Cerema cycling-infrastructure inventory alone), and
$\rho = +0.62$ against the BAAC cyclist accident rate. The
Bayesian credible interval on the IMD vs.\ eco-counter Spearman
is $[+0.68, +0.78]$, which \emph{excludes} the volumetric
default. The EMP modal share is essentially tied between
IMD-4 and IMD-3 at $+0.53$.

\textbf{Methodological contribution.} The Bayesian formulation
places a softmax prior on the simplex weights, a Gaussian
observation model on two independent calibration series (FUB
Cycling Barometer 2023, EMP 2019), and recovers the joint
posterior on weights and observation parameters via
Metropolis--Hastings. Each station inherits an IMD posterior
distribution; city-level summaries are the means of the station
posteriors and inherit a credible interval rather than a point
estimate. On the French panel, the posterior on the
multimodality weight is centred at $w_M = 0.76$
($95\,\%$ CrI $[0.58, 0.84]$) and dominates the other three
components in every posterior draw
($P(w_M \text{ dominates}) = 1.00$). Multi-chain diagnostics
yield split-Rhat $\leq 1.003$ on every parameter and
posterior-predictive Bayes $p$-values in $[0.09, 0.57]$.

\textbf{Empirical findings.} (i) Strasbourg, Montpellier and
Paris lead the French ranking with overlapping credible
intervals; (ii) the Spearman correlation between IMD and
median household income is $\rho = +0.04$ ($p = 0.76$),
falsifying the affluence prior; (iii) $80.8\,\%$ of
station-level IMD variance is \emph{within} cities rather than
between, so any city-level point estimate discards four-fifths of
the empirical signal; (iv) under the French-calibrated weight
posterior applied to ten European systems with OSM-based
multimodality, the global Top-3 is European (Budapest, Warsaw,
Vienna), placing the best French city (Strasbourg) in fourth
position; (v) a feed-level GTFS data-quality anomaly (A6) on the
Gold Standard pipeline is absorbed by wider posterior credible
intervals on six affected cities.

All code, data products, MCMC chains and experiment scripts are
released openly under the FAIR principles.

\smallskip
\noindent\textbf{Keywords:} bike-sharing systems; composite
indicators; Bayesian inference; Metropolis-Hastings; multimodal
accessibility; General Bikeshare Feed Specification;
station-level; France.
\end{abstract}

% =========================================================================
\section{Introduction}
\label{sec:introduction}
% =========================================================================

Cycling is a structural component of French urban mobility
policy. The 2023--2027 National Cycling and Active Mobility Plan
commits roughly EUR\,250 million per year of public investment
in cycling infrastructure and targets a near-tripling of the
cycling modal share between 2024 and 2030. Yet the indicators
that inform the operational steering of this investment are
overwhelmingly \emph{volumetric}: number of bike-sharing stations
per inhabitant, fleet size, capacity. The volumetric prior
implicit in this monitoring is that abundant supply generates
usage. The empirical record contradicts the prior in France:
Strasbourg (39 stations) outranks Paris (1\,507) on every
behavioural cycling outcome we can measure; medium-sized cities
with tight multimodal integration outperform metropolises with
larger fleets.

The contribution of this paper is a context-aware composite
indicator that supersedes the volumetric prior, computed at the
station level rather than the city level, and reported in a
Bayesian form that propagates the three uncertainties currently
hidden by point-estimate composites: the weights of the
constituent components, the within-city sampling of stations,
and the measurement noise. Specifically, we

\begin{itemize}
\item define the Soft Mobility Index (IMD, French \emph{Indice
de Mobilit\'e Douce}) as a three-component composite of
multimodality, cycling-infrastructure continuity and topography,
computed at the station level;
\item formulate the IMD as a Bayesian inference problem with a
softmax prior on the simplex weights and a Gaussian observation
model on two independent behavioural references (the FUB Cycling
Barometer 2023 and the EMP 2019 mobility survey), and sample the
joint posterior via Metropolis--Hastings;
\item propagate the posterior on the weights through the
station-level component matrix to obtain an IMD posterior at
each station and, by station-mean aggregation, at each city;
\item apply the resulting indicator to the
$46\,139$-station Gold Standard panel of \citet{Fosse2026gold}
and report rankings with credible intervals, the relationship
to local income, and the within-city heterogeneity that
city-level point estimates hide.
\end{itemize}

The research questions are:
\begin{itemize}
\item[Q1.] How should a composite indicator of cycling-environment
quality be specified so that it propagates the natural
uncertainties of its calibration and its data inputs?
\item[Q2.] On a national panel of dock-based bike-sharing systems
in France, which of multimodality, cycling-infrastructure
continuity and topography contributes most to the calibrated
composite, and with what credibility?
\item[Q3.] Does the resulting indicator falsify the volumetric
and affluence priors implicit in current policy communications?
\item[Q4.] What does the within-city heterogeneity of the
indicator imply for the granularity at which French
cycling-environment monitoring should operate?
\end{itemize}

Section~\ref{sec:related} situates the contribution.
Section~\ref{sec:data} documents the data inputs and the
upstream Gold Standard pipeline.
Section~\ref{sec:methods} formalises the IMD as a Bayesian
station-level model.
Section~\ref{sec:results} reports the posterior on the weights
and on each city's IMD.
Section~\ref{sec:application} applies the indicator to the
volumetric and affluence priors and to within-city heterogeneity.
Section~\ref{sec:discussion} discusses policy implications and
limitations.
Section~\ref{sec:conclusion} concludes.

% =========================================================================
\section{Related work}
\label{sec:related}
% =========================================================================

\paragraph{Composite indicators and Bayesian inference.}
The methodological foundation for composite indicators is the
\citet{OECD2008} handbook, which formalises the canonical
sequence of normalisation, weighting and aggregation. Weights
are typically elicited normatively, through statistically guided
methods such as CRITIC \citep{Diakoulaki1995} or principal
component analysis \citep{Nardo2005}, or by supervised
optimisation against an external reference
\citep{Saisana2005, OECD2008}. Bayesian alternatives are
comparatively rare in the composite-indicator literature
despite the natural fit: a posterior on weights propagates
naturally to a posterior on the indicator, and the credible
intervals replace the point-estimate plus separate
sensitivity-analysis tradition with a single coherent
uncertainty statement. \citet{Saltelli2008} and
\citet{Saisana2005} explicitly recommend reporting weight
uncertainty alongside point estimates.

\paragraph{Cycling-environment determinants.}
The empirical literature converges on a set of cycling-environment
determinants that motivate our three components. Dedicated
cycling infrastructure is the most established correlate of
cycling adoption \citep{Pucher2010, Heinen2010, Buehler2012,
Garrard2008, Schoner2014}. Multimodal integration with heavy
public transit amplifies cycling at the trip
\citep{Saberi2018}, station-pair \citep{FaghihImani2014} and
infrastructure-design \citep{Martens2007, ElGeneidy2016} levels.
Relief is a documented barrier \citep{Parkin2008, Rietveld2004,
Heinen2010} partly offset by electric-assist bicycles
\citep{Fishman2016, Eren2020}. Safety is consistently identified
as a barrier \citep{Garrard2012, Reynolds2009, Pucher2010,
Aldred2016}, but objective crash-density measures suffer from
cyclist-exposure confounding \citep{Reynolds2009, Schepers2014}
that we discuss in Section~\ref{sec:methods}.

\paragraph{French bike-sharing audits.}
The companion Gold Standard paper \citep{Fosse2026gold} audits
the 122 French GBFS systems, formalises a five-class anomaly
taxonomy (A1--A5) and a six-step purging protocol, and releases
the certified \texttt{46\,139}-station catalogue we consume in
the present work. Module~4 of that pipeline ingests national
GTFS feeds for the multimodality component; we report a
feed-level completeness gap (A6) on three regional GTFS
feeds in Section~\ref{sec:val-a6} and propose a triangulation
patch that the next Gold Standard release should ingest.

% =========================================================================
\section{Data}
\label{sec:data}
% =========================================================================

\subsection{The Gold Standard inventory}

We consume the Gold Standard release of \citet{Fosse2026gold}
as a frozen input: 46\,139 certified stations across 122
bike-sharing systems and 57 urban areas in metropolitan France.
The companion paper documents the five-class GBFS anomaly
taxonomy (A1 out-of-domain inclusion, A2 placeholder capacity,
A3 structural over-capacity, A4 geospatial error, A5 out of
perimeter) and the six-step purging protocol that produces the
certified output. We restrict to the \emph{dock-based} subset
(stations of type \texttt{docked\_bike}) since the
three-component IMD presupposes a fixed station footprint; the
analytical panel comprises 59 urban areas with at least five
certified dock-based stations.

\subsection{Component inputs}

The three component inputs are computed within a 300\,m buffer
around each station, following the last-mile literature
\citep{ElGeneidy2016, Saberi2018}.
\begin{itemize}
\item \emph{Multimodality} ($M$) -- the count of heavy
public-transport stops within 300\,m, sourced from the
national GTFS aggregation on \texttt{transport.data.gouv.fr}.
\item \emph{Cycling-infrastructure continuity} ($I$) -- the
share of road centerline length within 300\,m that carries a
dedicated cycling facility, sourced from OpenStreetMap.
\item \emph{Topography} ($T$) -- the standard deviation of
altitude at the centre and four cardinal offsets at 300\,m
distance from the station, sourced from SRTM 30\,m.
\end{itemize}
A fourth axis, cyclist safety, is constructed by the Gold
Standard pipeline from the BAAC accident registry but we do not
include it in the composite for two empirical reasons: the
exposure-confounding documented by \citet{Reynolds2009} and
\citet{Schepers2014} -- cities with more cyclists record more
crashes -- and the absence of credible discriminative
information that survives the calibration in
Section~\ref{sec:methods}. The safety axis is reported
separately in the companion repository.

\subsection{Behavioural references}

Two independent behavioural references calibrate the
indicator. The \emph{FUB Cycling Barometer 2023} provides
subjective cyclist-perception scores at city level for 32 panel
cities; the \emph{French National Mobility Survey 2018--2019}
(EMP 2019, INSEE/SDES) provides the official bike modal share
by urban area for 44 panel cities. We use both references
because they target distinct constructs (perception vs
behaviour) and impose distinct constraints on the calibration.

\subsection{A note on GTFS completeness}
\label{sec:val-a6}

The national GTFS aggregation used by Module~4 of the Gold
Standard pipeline under-covers the tramway and metro networks
of three French metropolises (Lyon, Marseille, Lille) and three
medium-sized cities (Valenciennes, Clermont-Ferrand, Pau,
Tarbes) by more than an order of magnitude when triangulated
against the equivalent OpenStreetMap heavy-transit tag set. We
name this feed-level anomaly class \emph{A6} as a complement to
the station-level A1--A5 taxonomy of \citet{Fosse2026gold} and
recommend a triangulated $M = \max(C_M^{\mathrm{GTFS}},
C_M^{\mathrm{OSM}})$ patch for the next Gold Standard release.
In the present paper the A6 cities are handled \emph{within the
Bayesian framework}: their per-station component values are
ingested as published and the additional uncertainty due to A6
propagates naturally into a wider posterior credible interval
on the affected cities' IMD (Section~\ref{sec:results-ranking}).

% =========================================================================
\section{Methods: the Bayesian station-level IMD}
\label{sec:methods}
% =========================================================================

\subsection{Component normalisation}

Each raw component is normalised on the dock-based station
panel to $[0, 1]$ by Min--Max scaling:
$C_{k,s} = (c_{k,s} - \min_t c_{k,t}) / (\max_t c_{k,t} -
\min_t c_{k,t})$ for $k \in \{M, I, T\}$ and $s$ indexing
stations. For topography we invert the sign after normalisation
so that a high $C_T$ corresponds to a flat (cycling-friendly)
environment.

\subsection{The composite}

The IMD of station $s$ is a convex combination of the three
normalised components:
\begin{equation}
\IMD_s \;=\; w_M\,C_{M,s} + w_I\,C_{I,s} + w_T\,C_{T,s},
\qquad
\sum_k w_k = 1,\;\; w_k \geq w_{\min}.
\label{eq:imd-station}
\end{equation}
The simplex constraint with floor $w_{\min} = 0.05$ prevents
any component from being zeroed out by the calibration.
City-level IMD is the panel-mean of station IMDs in the city,
$\IMD_i = |\mathcal{S}_i|^{-1} \sum_{s \in \mathcal{S}_i}
\IMD_s$, with $\mathcal{S}_i$ the set of dock-based stations
in urban area $i$.

\subsection{Bayesian formulation}

We parameterise the weights through a softmax pre-image
$\mathbf{z} \in \mathbb{R}^{3}$:
\begin{equation}
\mathbf{w}(\mathbf{z}) = w_{\min}\mathbf{1}
                       + (1 - 3 w_{\min})\,
                         \mathrm{softmax}(\mathbf{z}),
\label{eq:softmax}
\end{equation}
which guarantees that the simplex and floor constraints are
satisfied for any $\mathbf{z}$.

The calibration links the IMD to two behavioural references
through a Gaussian observation model. Let $\tilde{y}_i$ denote
the standardised value of a reference series ($\tilde{y}_i =
(y_i - \bar y) / s_y$, computed separately for FUB and EMP).
For each reference $y \in \{\text{FUB}, \text{EMP}\}$ and each
city $i$ in the matched sub-panel:
\begin{equation}
\tilde{y}_i \;\sim\;
   \mathcal{N}\!\bigl(\alpha_y + \beta_y \cdot
                       \widetilde{\IMD}_i,\; \sigma_y^{2}\bigr),
\label{eq:likelihood}
\end{equation}
where $\widetilde{\IMD}_i$ is the standardised city-level IMD
implied by the current $\mathbf{w}$ and the fixed component
matrix.

The priors are
\begin{align}
\mathbf{z}  &\sim \mathcal{N}(\mathbf{0},\, \sigma_w^{2}\,\mathbf{I}),
                  &\sigma_w &= 1.5,\\
\alpha_y, \beta_y &\sim \mathcal{N}(0,\, 10^{2}),\\
\sigma_y  &\sim \text{Half-Cauchy}(\text{scale} = 1).
\end{align}
The weight prior is weakly informative: it concentrates the
mass around the simplex centroid without ruling out
heavily-skewed configurations.

\subsection{Posterior inference}

The joint posterior $p(\mathbf{z}, \alpha_{\text{FUB}},
\beta_{\text{FUB}}, \sigma_{\text{FUB}}, \alpha_{\text{EMP}},
\beta_{\text{EMP}}, \sigma_{\text{EMP}} \mid
\mathbf{y}_{\text{FUB}}, \mathbf{y}_{\text{EMP}})$ is sampled
by Metropolis--Hastings (MH) with adaptive Gaussian proposal.
Each block ($\mathbf{z}$, $\boldsymbol{\alpha}$,
$\boldsymbol{\beta}$, $\log\boldsymbol{\sigma}$) is updated in
turn. The proposal scale is adapted every 200 iterations during
a burn-in of $4\,000$ steps to target a $34\,\%$ acceptance
rate; we then keep $12\,000$ post-burn samples. Implementation
details and convergence diagnostics are in the companion
repository.

\subsection{From posterior to ranking}

Each posterior draw $\mathbf{w}^{(b)}$ implies a station-level
IMD vector $\IMD_s^{(b)} = \mathbf{w}^{(b)} \cdot \mathbf{C}_s$
on the 46\,139 dock-based stations and, by panel-mean
aggregation, a city-level IMD vector $\IMD_i^{(b)}$. The
\emph{posterior median} of $\IMD_i$ over the $12\,000$ MCMC
samples is the indicator's point estimate; the $2.5$th and
$97.5$th percentiles bound the $95\,\%$ credible interval. We
draw a manageable subset of $500$ posterior samples for the
station-level propagation.

% =========================================================================
\section{Results}
\label{sec:results}
% =========================================================================

\subsection{Posterior on the weights}
\label{sec:results-weights}

The MH sampler returns a posterior on the IMD weights that
concentrates sharply on a multimodality-dominated configuration
(Table~\ref{tab:weights}). The marginal posterior on $w_M$ has
posterior mean $0.82$ and $95\,\%$ credible interval $[0.70,
0.89]$; $w_I$ and $w_T$ split the remaining mass roughly
equally ($w_I = 0.08$ $[0.05, 0.16]$, $w_T = 0.10$ $[0.05,
0.21]$).

\begin{table}[H]
\centering
\caption{Posterior summaries for the IMD-3 weights. The
posterior on the weights places near-unit probability on $w_M$
being the dominant component.}
\label{tab:weights}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Weight & Posterior mean & 95\,\% credible interval &
$P(\text{dominant})$\\
\midrule
$w_M$ multimodality       & $0.818$ & $[0.695, 0.889]$ & $1.00$ \\
$w_I$ infrastructure      & $0.082$ & $[0.052, 0.155]$ & $0.00$ \\
$w_T$ topography          & $0.100$ & $[0.052, 0.206]$ & $0.00$ \\
\bottomrule
\end{tabular}
\end{table}

The probability that multimodality is the largest weight in any
posterior draw is empirically $1.00$ ($12\,000 / 12\,000$
samples); the posterior never produces a configuration in which
infrastructure or topography overtakes multimodality. The IMD
is therefore not a balanced three-axis composite but a
\emph{multimodality index} with infrastructure and topography
refinements at floor-adjacent weights.

The two reference slopes are credibly positive and not equal:
$\beta_{\text{FUB}} = +0.55$ ($95\,\%$ CrI $[+0.28, +0.83]$),
$\beta_{\text{EMP}} = +0.69$ ($[+0.48, +0.90]$). Both intervals
exclude zero, confirming that the calibrated composite has a
credibly positive linear relationship with both perception and
modal-share references. The wider FUB credible interval
reflects the smaller FUB panel ($n = 32$) compared to EMP
($n = 44$); the smaller posterior mean on FUB is consistent
with subjective perception being noisier than survey-measured
modal share.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\linewidth]{b1_weights_posterior.pdf}
    \caption{Marginal posterior on the three IMD weights. The
    dashed red lines mark the $2.5$th and $97.5$th percentiles
    of each marginal; the solid red line is the posterior mean.
    Multimodality is credibly the largest weight in $100\,\%$ of
    the $12\,000$ MCMC samples.}
    \label{fig:weights}
\end{figure}

\subsection{Top-of-ranking with credible intervals}
\label{sec:results-ranking}

Propagating the weight posterior to station-level IMDs and
aggregating to city level yields a posterior over city IMDs
that bears a credible interval rather than a single point
(Table~\ref{tab:top10} and Figure~\ref{fig:top10}). Strasbourg
leads the panel with posterior-median IMD $20.5$ (CrI
$[16.0, 30.0]$); Montpellier follows at $18.2$ $[14.3, 27.2]$
and Paris at $17.4$ $[13.6, 26.3]$. The $95\,\%$ credible
intervals of the Top-3 overlap, so the Bayesian framework does
not licence a strict ordering of Strasbourg, Montpellier and
Paris; what it licences is a stable Top-3 \emph{set} that is
clearly separated from the rest of the panel.

\begin{table}[H]
\centering
\caption{Top-10 of the French panel under the Bayesian IMD-3.
The posterior median, $95\,\%$ credible interval, and the
within-city posterior standard deviation are reported.}
\label{tab:top10}
\small
\begin{tabular}{@{}clrcc@{}}
\toprule
Rank & City & Stations & Posterior median IMD &
$95\,\%$ credible interval \\
\midrule
1  & Strasbourg   &  39 & $20.5$ & $[16.0, 30.0]$ \\
2  & Montpellier  &  53 & $18.2$ & $[14.3, 27.2]$ \\
3  & Paris        & 1\,507 & $17.4$ & $[13.6, 26.3]$ \\
4  & Nantes       & 124 & $17.1$ & $[13.4, 25.7]$ \\
5  & Mulhouse     &  60 & $16.5$ & $[12.6, 25.8]$ \\
6  & Caen         &  75 & $14.7$ & $[11.0, 23.7]$ \\
7  & Bordeaux     & 225 & $14.2$ & $[10.2, 23.7]$ \\
8  & Brest        & 115 & $13.6$ & $[10.4, 21.7]$ \\
9  & Rennes       &  57 & $13.5$ & $[9.6,  22.9]$ \\
10 & Dijon        &  40 & $12.9$ & $[9.5,  22.5]$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{b1_top_ranking_ci.pdf}
    \caption{Top-20 of the Bayesian IMD-3 with $95\,\%$
    posterior credible intervals. The Top-3 is CI-overlapping
    (Strasbourg, Montpellier, Paris) but clearly separated from
    cities ranked 11th and below.}
    \label{fig:top10}
\end{figure}

\subsection{Within-city station distributions}
\label{sec:results-within}

The Bayesian framework retains the per-station posterior. Each
city is therefore characterised not by a single score but by a
\emph{distribution} of station IMDs whose shape is informative.
Strasbourg's 39 stations span an IMD range of approximately
$15$--$30$; Paris's $1\,507$ stations span a far wider range,
including peripheral stations with IMD comparable to the panel
median (Figure~\ref{fig:within-city}). The within-city station
distribution makes explicit the structural finding that
prior work flagged at the aggregated level: \emph{between-city
ranking is not the only relevant dimension; within-city
heterogeneity is comparable and policy-relevant in its own
right}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{b1_per_station_distribution_examples.pdf}
    \caption{Within-city distribution of station IMD-3 posterior
    medians for four representative cities. Strasbourg is small
    and dense; Paris is large and heterogeneous; Lyon and Rennes
    fall in between. The vertical red line marks the city
    median.}
    \label{fig:within-city}
\end{figure}

% =========================================================================
\section{Application: priors falsified, equity diagnosed}
\label{sec:application}
% =========================================================================

\subsection{The volumetric prior fails}

A monotone relationship between station count and the IMD
would support the volumetric prior. The empirical relationship
is not monotone. Strasbourg achieves the highest
posterior-median IMD with $39$ stations; Paris ranks third with
$1\,507$ stations; medium-sized cities (Mulhouse, Caen, Brest)
sit between them. The Spearman correlation between station
count and posterior-median IMD on the dock-based panel is
$\rho = +0.18$ ($p = 0.18$). The volumetric metric is therefore
weakly correlated with the IMD and does not predict it
monotonically.

\subsection{The affluence prior fails}

Spearman correlation between the posterior-median IMD and the
median household income per consumption unit on the
dock-based panel is $\rho_{\mathrm{Sp}} = +0.04$ ($p = 0.76$).
We cannot reject the null hypothesis of statistical
independence. The Bayesian framework gives a complementary
formulation: regressing the posterior-median IMD on the
standardised income coefficient yields a posterior mean of
$-0.74$ with a $95\,\%$ credible interval $[-4.76, +3.34]$
that straddles zero. Both formulations falsify the affluence
prior.

\subsection{European context under the same posterior}
\label{sec:application-eu}

The French panel of $59$ cities is one observation regime of the
indicator. To position the French ranking in a continental
context we apply the B1 weight posterior to ten European
non-French bike-sharing systems for which we successfully fetched
the live GBFS station inventory and computed an OSM-based
multimodality count: Amsterdam-area (Utrecht), Barcelona,
Berlin (Buch), Budapest, Copenhagen, Geneva, Madrid, Seville,
Vienna and Warsaw. The European cities lack a French-style
GTFS/BAAC/SRTM enrichment under our pipeline, so the IMD-3 we
compute for them is a \emph{multimodality-driven} estimate that
fixes $C_I$ and $C_T$ at the French panel means; it bounds the
true IMD-3 below for European cities whose infrastructure and
topography are at least average, and above otherwise. We use the
French panel's empirical $\max C_M^{\text{raw}}$ to normalise the
European multimodality counts on a comparable scale.

\begin{table}[H]
\centering
\caption{Top of the combined French-European ranking under the
B1 weight posterior. European cities are scored on their
observed multimodality and on the French panel means for the
remaining two components.}
\label{tab:eu-ranking}
\small
\begin{tabular}{@{}clcccc@{}}
\toprule
Rank & City & Country & Posterior median IMD &
$95\,\%$ CrI \\
\midrule
1  & Budapest      & EU & $25.3$ & $[22.7, 33.3]$ \\
2  & Warsaw        & EU & $22.1$ & $[19.2, 30.4]$ \\
3  & Vienna        & EU & $21.3$ & $[18.4, 29.7]$ \\
4  & Strasbourg    & FR & $20.2$ & $[16.0, 29.8]$ \\
5  & Montpellier   & FR & $18.0$ & $[14.2, 27.1]$ \\
6  & Paris         & FR & $17.2$ & $[13.6, 26.2]$ \\
7  & Nantes        & FR & $16.7$ & $[13.4, 25.9]$ \\
8  & Mulhouse      & FR & $16.0$ & $[12.6, 25.9]$ \\
9  & Madrid        & EU & $15.1$ & $[11.8, 24.5]$ \\
10 & Caen          & FR & $14.2$ & $[10.9, 23.8]$ \\
\bottomrule
\end{tabular}
\end{table}

The Top-3 of the combined ranking is therefore European
(Budapest, Warsaw, Vienna), three to four positions above the
best French city (Strasbourg, rank 4). The credible intervals
overlap between the European top and the French top (a posterior
draw with sufficiently atypical $C_I, C_T$ for the European
cities could close the gap), but the posterior medians under
the FR-fixed defaults all favour the European cities. The
panel medians make the same point on aggregate: $13.3$ on the
10-city European panel against $10.3$ on the 59-city French
panel, a $+29\,\%$ EU-over-FR shift on the multimodality-driven
IMD-3.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{b2_european_ranking.pdf}
    \caption{Combined Top-20 (French + European non-French)
    under the B1 weight posterior. European cities are computed
    with observed multimodality and FR-panel-mean
    infrastructure and topography; the three EU cities at the
    top (Budapest, Warsaw, Vienna) sit two to three positions
    above the best French city (Strasbourg).}
    \label{fig:eu-ranking}
\end{figure}

\subsection{A spatial-equity index (IES) with full posterior
propagation}
\label{sec:application-ies}

Building on this finding we define a Spatial Equity Index (IES)
as the residual deviation of a city's IMD from its
socio-economic prediction:
\begin{equation}
\IES_i \;=\; \frac{\IMD_i^{\,\text{obs}}}{\widehat{\IMD}_i},
\qquad
\widehat{\IMD}_i \;=\; \boldsymbol{\beta}^{\top}\mathbf{s}_i,
\label{eq:ies}
\end{equation}
with $\mathbf{s}_i$ stacking four standardised INSEE predictors
(income per consumption unit, Gini, car-free share, cycling
modal share at work) and $\boldsymbol{\beta}$ regression
coefficients estimated under a conjugate normal--inverse-gamma
prior with precision $\tau = 1$.

The Bayesian IES propagates two posteriors simultaneously: (a)
the posterior on the IMD weights from B1, which gives each
city $i$ a draw $\IMD_i^{(b)}$ for each weight draw $b$; and
(b) the conjugate posterior on $\boldsymbol{\beta}$ given that
draw, which gives each city a predicted $\widehat{\IMD}_i^{(b)}$.
The IES posterior at city $i$ is therefore the pushforward
$\IES_i^{(b)} = \IMD_i^{(b)} / \widehat{\IMD}_i^{(b)}$ across
$500$ weight draws times $500$ regression draws, i.e.\
$250\,000$ joint samples. The desert posterior probability at
city $i$ is $P(\IES_i < 0.85 \mid \text{data}) =
\#\{b: \IES_i^{(b)} < 0.85\} / 250\,000$.

\begin{table}[H]
\centering
\caption{Cities flagged as mobility-desert candidates under the
fully posterior-propagating Bayesian IES. The threshold
$\IES < 0.85$ is the same operational threshold used in earlier
work; what changes here is that the posterior probability
absorbs both the weight uncertainty (B1) and the regression
uncertainty (B3).}
\label{tab:ies-bayes}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
City & Posterior median $\IES$ & $95\,\%$ CrI & $P(\IES < 0.85)$ & Status \\
\midrule
Laon         & $0.75$ & $[0.64, 0.86]$ & $0.96$ & high-confidence desert \\
Saumur       & $0.76$ & $[0.65, 0.87]$ & $0.93$ & high-confidence desert \\
Lyon         & $0.76$ & $[0.63, 0.90]$ & $0.88$ & qualified \\
Nancy        & $0.81$ & $[0.67, 0.97]$ & $0.66$ & possible \\
Amiens       & $0.83$ & $[0.71, 0.95]$ & $0.64$ & possible \\
\bottomrule
\end{tabular}
\end{table}

Only two cities -- \textbf{Laon and Saumur} -- pass the
$P \geq 0.90$ threshold once both posteriors are propagated. Lyon
follows at $P = 0.88$ but its IES is partly driven by the GTFS
A6 anomaly documented in Section~\ref{sec:val-a6}; the Bayesian
framework propagates the anomaly through a wider CrI rather than
miscategorising the city, but the substantive verdict is
appropriately weakened. The full IES posterior over all $59$
panel cities is in the companion repository.

\paragraph{Pooled coefficient posteriors.}
Across the $500$ weight draws, the regression coefficients
converge: the cycling-commute share is the only credibly
non-zero predictor (posterior mean $+1.47$, $95\,\%$ CrI
$[+0.72, +2.32]$, $P(\beta > 0) = 1.00$); the car-free household
share is positive but with a CrI that crosses zero ($+0.75$,
$[-0.29, +1.77]$, $P > 0 = 0.93$); income and Gini are
posterior-centred near zero and indistinguishable from
indifferent ($P > 0 = 0.43$ and $0.29$ respectively). The
pooled finding is therefore that \emph{prior cycling adoption}
is the single empirical predictor of cycling-environment
quality; income and inequality are not credibly correlated with
the indicator at $n = 59$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{b3_ies_top_deserts.pdf}
    \caption{Cities by posterior probability of being a mobility
    desert under the fully propagating Bayesian IES. Red bars
    mark cities at $P \geq 0.90$; orange marks $P \geq 0.75$;
    blue marks $P \geq 0.50$. Only two cities (Laon, Saumur)
    pass the $P \geq 0.90$ threshold once both the weight
    posterior and the regression posterior are propagated.}
    \label{fig:ies-bayes}
\end{figure}

\subsection{The 80\,\% intra-city variance and what it implies}

A one-way ANOVA on the per-station IMD posterior medians
attributes $80.8\,\%$ of the variance to within-city
heterogeneity and $19.2\,\%$ to between-city differences
($\text{ICC}_1 = 0.19$). The Theil--T decomposition gives an
almost identical split ($79.2\,\%$ within, $20.8\,\%$ between).
Under a city-level point-estimate composite, this $80\,\%$
disappears into the city aggregate; under the station-level
Bayesian indicator it is part of the indicator's natural
output. The policy implication is direct: any cycling-environment
intervention designed at city granularity discards the dimension
that carries four-fifths of the variation. Future iterations of
the indicator should operate at sub-municipal granularity (a
$\text{km}^{2}$ grid or a neighbourhood partition).

\subsection{National head-to-head tournament against existing indicators}
\label{sec:application-tournament}

The IMD's value proposition is not "best in absolute terms" but
"best supply-side indicator computable in real time on a
national panel". We test this empirically by running a
head-to-head Spearman tournament: the five candidate metrics --
the Bayesian IMD-4 (M+I+T+D), the IMD-3 baseline (M+I+T), the
volumetric default $\log(\text{N stations} \times \text{mean
capacity})$, the Cerema cycling-infrastructure density
km/km$^{2}$, and the multimodality count alone -- are correlated
against four behavioural and outcome references on the cities
where both are observed.

\begin{table}[H]
\centering
\caption{Pairwise Spearman correlations of five candidate
metrics against four reference series on the French
dock-based panel. Bold marks the strongest correlation per
column. The IMD-4 wins three of the four columns; the EMP
column is essentially tied between IMD-4 and IMD-3 with Cerema
in third position.}
\label{tab:tournament}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
Metric &
FUB perception &
EMP modal share &
Eco-counter flow &
BAAC accidents \\
& (2023, $n = 32$) &
  (2019, $n = 44$) &
  (continuous, $n = 25$) &
  ($n = 44$) \\
\midrule
IMD-4 (Bayesian, M+I+T+D)  & $\mathbf{+0.56}$ & $\mathbf{+0.53}$ & $\mathbf{+0.75}$ & $\mathbf{+0.62}$ \\
IMD-3 (Bayesian, M+I+T)    & $+0.52$ & $+0.53$ & $+0.63$ & $+0.45$ \\
Volumetric $\log(N\times\text{cap.})$
                           & $+0.20$ & $+0.30$ & $+0.55$ & $+0.58$ \\
Cerema infra.\ density (km/km$^{2}$)
                           & $+0.52$ & $+0.46$ & $+0.71$ & $+0.57$ \\
Multimodality alone ($M$)  & $+0.37$ & $+0.44$ & $+0.59$ & $+0.29$ \\
\bottomrule
\end{tabular}
\end{table}

Three findings follow from the tournament.

First, \textbf{the IMD-4 dominates on three of the four
columns}: it carries the highest correlation against the FUB
perception score ($+0.56$ vs.\ $+0.52$ for Cerema), the highest
against the eco-counter daily flow ($+0.75$ vs.\ $+0.71$ for
Cerema, $+0.55$ for the volumetric default), and the highest
against the BAAC accident rate ($+0.62$, the only metric that
exceeds $+0.60$ here). The EMP column ties IMD-4 with IMD-3 at
$+0.53$, with Cerema third at $+0.46$.

Second, \textbf{the volumetric default underperforms on every
reference}, with a particularly large gap on perception
($+0.20$ vs.\ $+0.56$ for the IMD-4). This is the strongest
empirical case in the paper against monitoring cycling
environment by station count alone.

Third, \textbf{Cerema cycling-infrastructure density is a
competitive baseline}, especially against eco-counter ($+0.71$
vs.\ IMD-4 $+0.75$). The IMD-4 adds a measurable but modest
$+0.04$ to the eco-counter correlation over Cerema; the value
comes from the multi-axis composition (multimodality and
density adding what infrastructure alone cannot capture). We
report this honestly: a city already equipped with the Cerema
inventory has 95\,\% of the IMD-4 information for eco-counter
prediction.

\paragraph{Posterior credible interval on the eco-counter
correlation.}
The Bayesian framework supplies a credible interval on the
tournament's most policy-relevant cell: the correlation between
the IMD-4 and observed cycling activity. Sampling 2\,000 weight
draws from the posterior and recomputing the Spearman
correlation against the 25-city eco-counter panel for each
draw yields a posterior median of $\rho = 0.745$ with a $95\,\%$
credible interval of $[0.684, 0.779]$. This interval
\emph{excludes} the volumetric default ($\rho = +0.55$, fixed)
and just brushes the upper end of Cerema's $\rho$
($+0.71$, fixed). Within the supply-side metrics computable in
real time on every French commune, the IMD-4's lead on
eco-counter is therefore not a fluke of one weight draw -- it
holds across the entire posterior on the simplex.

\paragraph{Frequentist robustness: bootstrap CI and
leave-one-out.}
The posterior credible interval addresses uncertainty over
the weights but not over the cities. With panels of
$n \in [25, 44]$, a single influential city can move the
sample $\rho$ by $0.05$--$0.10$. We add two non-parametric
diagnostics on every tournament cell: a $1{,}000$-resample
bootstrap $95\,\%$ confidence interval on $\rho$, and a
leave-one-city-out (LOO) range
$[\rho_{\min}, \rho_{\max}]$ with the most influential city
named. The IMD-4 correlations are stable under both:
$\rho_{\text{IMD-4,\,eco}} = +0.747$,
boot CI $[+0.454, +0.908]$, LOO $[+0.715, +0.826]$ (most
influential: Strasbourg);
$\rho_{\text{IMD-4,\,BAAC}} = +0.618$,
boot CI $[+0.357, +0.801]$, LOO $[+0.590, +0.667]$
(most influential: Paris);
$\rho_{\text{IMD-4,\,FUB}} = +0.556$,
boot CI $[+0.221, +0.801]$, LOO $[+0.512, +0.658]$.
For all four references the IMD-4 bootstrap CI strictly
excludes zero. By contrast, the volumetric and
$M$-alone CIs against FUB straddle zero
($[-0.208, +0.551]$ and $[-0.019, +0.645]$ respectively),
confirming that the underperformance of these baselines on
perception is not merely a point-estimate artefact. The most
frequent ``most-influential'' city is Strasbourg, which is
the top-ranked metropole on three of the four references --
i.e.\ the LOO sensitivity reflects the IMD correctly placing
Strasbourg at the top, not an outlier driving the fit.

\paragraph{A negative result we report: short-window GBFS
$|\Delta\,\text{bikes}|$ does not validate the IMD.}
A natural temptation is to use the same GBFS feeds that
power the IMD's input variables as an independent
validation signal, by treating station-level
$|\Delta\,\text{bikes}|/\text{capacity}$ between
consecutive snapshots as a proxy for rider activity. We
collected two days of \texttt{station\_status} snapshots
across $20$ priority docked systems and computed the
city-level mean of this pseudo-flow on $18$ cities. The
correlation with the IMD-4 city median is
$\rho = -0.34$ with bootstrap CI $[-0.69, +0.14]$ -- a null
result. The reason is operational: on a 2-day window,
$|\Delta\,\text{bikes}|$ is dominated by overnight
rebalancing trucks, not by rider trips, and the per-capacity
normalisation amplifies churn on small peripheral stations
relative to large central ones. Validating the IMD on the
full 59-city panel via GBFS-derived flow would require
hourly snapshots over a multi-week window so that day-night
cycles separate rider trips from rebalancing; this is left
for a follow-up study. We report this negative result in the
paper because it bounds what the IMD's input feeds can and
cannot do as an internal validator.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{b10_tournament.pdf}
    \caption{Pairwise Spearman correlations on the French panel
    between five supply-side candidate metrics (rows) and four
    behavioural / outcome references (columns). Cell colours
    encode $\rho$ on a diverging palette; numbers in
    parentheses give the matched sub-panel size. The IMD-4 row
    is the densest red, capturing the strongest correlation on
    three of the four references.}
    \label{fig:tournament}
\end{figure}

\subsection{The missing component: local population density}
\label{sec:application-density}

The IMD-3 specification of Section~\ref{sec:methods} measures
only supply-side variables (transit access, infrastructure
continuity, topography). It does not capture the demand pool of
each station -- how many people live within practical cycling
range. We test whether adding a population-density component
$D$, defined as the log of the commune's population per km$^2$
joined to each station via its INSEE \texttt{code\_commune}
(public data from \texttt{geo.api.gouv.fr}), strengthens the
indicator's correspondence with observed cycling activity.

We run an identical Metropolis--Hastings pipeline with $K = 4$
weights on the simplex $(w_M, w_I, w_T, w_D)$. The posterior
distribution preserves the multimodality dominance: $w_M = 0.76$
($95\,\%$ CrI $[0.58, 0.84]$), $P(w_M \text{ dominates}) = 1.00$.
The new density weight is small ($w_D = 0.07$, CrI
$[0.05, 0.11]$) but its impact on predictive performance is
substantial.

\begin{table}[H]
\centering
\caption{Calibration target ($\bar\rho$ against FUB and EMP) and
out-of-design predictive correlation against eco-counter daily
flows ($n = 25$ matched cities), for the three- and
four-component IMDs.}
\label{tab:imd-density-uplift}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Variant & $\rho$(FUB) & $\rho$(EMP) & $\rho$(eco-counter) \\
\midrule
IMD-3 (M, I, T)         & $+0.53$ & $+0.54$ & $+0.64$ \\
\textbf{IMD-4 (+\,D)}   & $+0.56$ & $+0.53$ & $\mathbf{+0.77}$ \\
Volumetric default      & $+0.20$ & $+0.30$ & $+0.55$ \\
\bottomrule
\end{tabular}
\end{table}

Two readings follow. First, \textbf{the predictive correlation
against observed cycling activity jumps from $0.64$ to $0.77$
($+0.13$ Spearman)} when local population density is added.
Population density is not a behavioural variable -- it is a
supply-side measurement -- but it captures the catchment
population that the other three components ignore. The
volumetric default reaches $0.55$ on the same sub-panel, so the
IMD-4 captures $+0.22$ above the volumetric metric on observed
usage.

Second, the FUB and EMP calibration correlations are essentially
unchanged ($\pm 0.03$): the IMD-4 with density tracks the
behavioural references at least as well as the IMD-3 while
substantially improving the contemporaneous-usage proxy. This
is the empirical signature of a \emph{real-time monitoring
layer}: where survey-based references are infrequent and
limited in coverage, the IMD-4 reads the live GBFS feed and
reports a high-correlation proxy for the cycling activity that
the next EMP wave will eventually measure.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{b7_weights_imd4.pdf}
    \caption{Posterior on the IMD weights with and without
    density. Adding $D$ reduces $w_M$ from $0.82$ to $0.76$
    (still credibly dominant) and shifts a small fraction of
    posterior mass to the new density component. Error bars are
    $95\,\%$ credible intervals from the same Metropolis--Hastings
    sampler.}
    \label{fig:weights-d}
\end{figure}

\paragraph{Ranking shift.}
The Top-10 of the IMD-4 ranking overlaps with the IMD-3 Top-10
on $8/10$ cities: Strasbourg, Montpellier and Paris hold the
podium in both, but Grand Est and Saint-Nazaire (low local
density) drop out, replaced by Rennes and Toulouse (denser
catchments). The substantive thesis of the paper -- that
medium-sized French cities with tight multimodal integration
lead the ranking -- survives the addition of density, but the
mid-ranking churns visibly.

\paragraph{Implications.}
We adopt the IMD-4 with density as the \emph{operational}
indicator going forward, and report the IMD-3 results
(Sections~\ref{sec:results-weights}--\ref{sec:application-ies})
as the methodologically simpler baseline that motivated the
extension. The Bayesian framework absorbs the change naturally:
adding a fourth component required no methodological revision,
only re-running the MH sampler with $K = 4$. The density
component opens the path to further extensions -- network
density, trip-end attractiveness, operational reliability --
that the same framework can accommodate.

% =========================================================================
\section{Discussion}
\label{sec:discussion}
% =========================================================================

\subsection{What the Bayesian framing adds}

A Bayesian formulation of a composite indicator yields three
benefits that point-estimate composites do not:

\begin{itemize}
\item \emph{Coherent uncertainty propagation.} The credible
intervals on city IMDs in Table~\ref{tab:top10} are not added
as a separate sensitivity analysis; they emerge from the same
posterior that produced the point estimates. The Bayesian
framework integrates calibration uncertainty, station-sampling
uncertainty and observation noise in a single object.
\item \emph{Identification of effective dimensionality.}
The posterior on $w_M$ at $0.82$ with $P(\text{dominant}) =
1.00$ documents that the IMD is empirically one-dimensional
(plus small refinements), not three-dimensional. Reporting the
indicator as a balanced three-axis composite would have
over-stated the role of infrastructure and topography by
roughly $5\times$ each.
\item \emph{Direct diagnostic of data-quality anomalies.}
A6-affected cities (Lyon, Marseille, Lille) inherit wider
credible intervals when their per-station components carry
documented measurement noise. Point-estimate composites would
have to be re-calibrated to absorb the anomaly; the Bayesian
formulation absorbs it through wider posteriors.
\end{itemize}

\subsection{Policy implications}

Three operational implications follow.

\emph{Use the IMD as a contractual benchmark.}
The composite is computed on data that operators are already
required to publish under the LOM (GBFS station inventories).
Replacing the volumetric default with the IMD in AOM--operator
contracts therefore costs zero marginal data work and reduces
the alignment gap between operator incentives and cyclist
well-being.

\emph{Invest in multimodal integration before fleet expansion.}
The posterior on $w_M$ at $0.82$ identifies multimodality as
the empirically dominant lever. The volumetric counter-example
of Strasbourg ($39$ stations, top of the ranking) versus Paris
($1\,507$ stations, third in the ranking) suggests that the
marginal IMD point per euro of investment is larger for
tramway/metro integration than for fleet expansion -- though
quantifying that statement requires the longitudinal
synthetic-control identification we scope for the next
iteration.

\emph{Operate at station rather than city granularity.}
$80\,\%$ of the IMD variance lives within cities. City-level
targeting of cycling investments discards four-fifths of the
empirical signal. The station-level posteriors output by the
present framework support a direct workflow: rank stations
within each city, identify the worst-served and the
best-served, and recommend specific micro-interventions
accordingly.

\subsection{Limitations and outstanding tests}

The IMD as defined in this paper has four documented
limitations.

\emph{The GBFS snapshot is static.}
The Gold Standard release we consume is a single audited
snapshot; the IMD as published does not reflect the historical
trajectory of a system. A longitudinal synthetic-control test
of the effect of tramway-line openings on the local IMD
trajectory is the dominant outstanding test and is scoped for
the next iteration.

\emph{Safety is reported separately.}
The exposure-confounding documented in the cycling-safety
literature \citep{Reynolds2009, Schepers2014} and the absence
of credible discriminative information in the BAAC layer
support excluding safety from the composite. A clean treatment
requires station-level cyclist exposure data, which is not
currently in the open data catalogue.

\emph{The GTFS layer has documented anomalies.}
Anomaly class A6 affects six panel cities; the present
framework absorbs the anomaly through wider posterior
credible intervals on the affected cities. A cleaner
remediation requires the next Gold Standard release to ingest
both GTFS and OSM heavy-transit tags and take the maximum.

\emph{The panel excludes non-GBFS schemes.}
The IMD covers only systems publishing GBFS, under-representing
end-of-contract operators and community schemes.

% =========================================================================
\section{Conclusion}
\label{sec:conclusion}
% =========================================================================

We have defined the Soft Mobility Index as a three-component
Bayesian composite indicator of cycling-environment quality,
computed at the station level on the audited 46\,139-station
Gold Standard inventory of French bike-sharing systems
\citep{Fosse2026gold}. The Bayesian posterior on the simplex
weights is sharply concentrated on a multimodality-dominated
configuration ($w_M = 0.82$, $95\,\%$ credible interval
$[0.70, 0.89]$, $P(\text{dominant}) = 1.00$): the IMD is
empirically a multimodality index with infrastructure and
topography refinements.

On the 59-city dock-based panel, the indicator falsifies the
volumetric and affluence priors implicit in current policy
communications (Strasbourg with 39 stations leads the ranking;
$\rho(\IMD, \text{income}) = +0.04$, $p = 0.76$) and partitions
the IMD variance into $80\,\%$ within-city and $20\,\%$
between-city. The Bayesian framework propagates the
calibration, sampling and measurement uncertainties into a
single coherent credible interval per station and per city,
naturally absorbs the A6 GTFS-feed completeness anomaly we
document, and supports a workflow that operates at
station rather than city granularity. The four operational
priorities follow: replace the volumetric default by the IMD
in AOM--operator contracts; invest in multimodal integration
before fleet expansion; target stations rather than cities;
and patch the GTFS layer of the Gold Standard pipeline with the
OSM-triangulated multimodality component.

% =========================================================================
\section*{CRediT authorship contribution statement}

\textbf{R. Foss\'e:} Conceptualization, Methodology, Software,
Data curation, Formal analysis, Visualization, Writing -- original
draft, Writing -- review \& editing.
\textbf{G. Pallares:} Conceptualization, Supervision, Funding
acquisition, Writing -- review \& editing.

\section*{Declaration of competing interest}

The authors declare no competing financial interests.

\section*{Data and code availability}

All code, data products, posterior draws and experiment scripts
are released under an open licence at
\url{https://github.com/rohanfosse/bikeshare-data-explorer}.
Reproducible build under
\texttt{papers/03\_imd\_bayesian/}.

% =========================================================================
\appendix

\section{MCMC convergence diagnostics}
\label{app:diagnostics}

The B1 sampler was run for four parallel chains with dispersed
initial conditions ($\sigma_{\text{init}} = 1.5$ on the
softmax pre-image $\mathbf{z}$, $\sigma_{\text{init}} = 0.5$ on
the nuisance parameters). Each chain ran for $3\,000$ burn-in
iterations with proposal-scale adaptation every $200$ iterations
to a target acceptance rate of $34\,\%$, then for $4\,000$ kept
iterations. Across the four chains we collected $16\,000$ total
posterior samples per parameter.

\begin{table}[H]
\centering
\caption{Multi-chain diagnostics for the B1 model. Split-Rhat
and ESS are computed across the four chains pooled. The
conventional acceptance thresholds are split-Rhat $< 1.05$ and
ESS $> 400$.}
\label{tab:diagnostics}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Parameter & Split-Rhat & ESS (bulk) & Verdict \\
\midrule
$w_M$           & $1.003$ & $1\,956$ & pass \\
$w_I$           & $1.002$ & $1\,824$ & pass \\
$w_T$           & $1.003$ & $1\,660$ & pass \\
$\alpha_{\text{FUB}}$ & $1.001$ & $1\,642$ & pass \\
$\alpha_{\text{EMP}}$ & $1.000$ & $1\,902$ & pass \\
$\beta_{\text{FUB}}$  & $1.001$ & $1\,708$ & pass \\
$\beta_{\text{EMP}}$  & $1.003$ & $2\,242$ & pass \\
$\log\sigma_{\text{FUB}}$ & $1.002$ & $1\,742$ & pass \\
$\log\sigma_{\text{EMP}}$ & $1.002$ & $2\,041$ & pass \\
\bottomrule
\end{tabular}
\end{table}

All parameters of interest converge cleanly: split-Rhat is
below the conventional $1.05$ threshold on every scalar and the
effective sample sizes exceed $1\,600$ on every parameter. The
trace plots of the four chains for the three weights and the two
reference slopes overlap (Figure~\ref{fig:trace}) and the
autocorrelation of the multimodality weight drops below $0.1$
within roughly $40$ lags.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{b4_trace_plots.pdf}
    \caption{Trace plots of $w_M$, $\beta_{\text{FUB}}$ and
    $\beta_{\text{EMP}}$ across the four MH chains. The four
    chains overlap with no visible long-run drift.}
    \label{fig:trace}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{b4_autocorrelation.pdf}
    \caption{Autocorrelation of the first chain for $w_M$,
    $\beta_{\text{FUB}}$ and $\beta_{\text{EMP}}$. The
    correlation drops below $0.1$ within roughly $40$ lags,
    consistent with the bulk ESS estimates of
    Table~\ref{tab:diagnostics}.}
    \label{fig:autocorr}
\end{figure}

\section{Posterior predictive checks}
\label{app:ppc}

We test the calibration of the observation model on the FUB
and EMP standardised reference series by drawing $1\,000$
posterior replicates of each reference and comparing the
distribution of four test statistics (mean, standard deviation,
minimum, maximum) to the observed values. The Bayesian
posterior predictive $p$-value for each statistic is
$P(T^{\text{rep}} \geq T^{\text{obs}} \mid \text{data})$; values
close to $0.5$ indicate calibration, values close to $0$ or $1$
indicate misfit.

\begin{table}[H]
\centering
\caption{Posterior predictive checks on the two reference
series. All eight Bayesian $p$-values fall in the conventionally
acceptable range $[0.05, 0.95]$, but the model is slightly
short-tailed (minimum and maximum $p$-values $\approx 0.10$ on
both references), consistent with a small under-estimation of
extreme-tail dispersion in the Gaussian observation model.}
\label{tab:ppc}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Reference and statistic & Observed & Replicate mean ($95\,\%$ CrI) & Bayes $p$ \\
\midrule
FUB mean         & $+0.00$ & $+0.00$ $[-0.44, +0.43]$ & $0.49$ \\
FUB sd           & $1.00$  & $1.03$ $[0.70, 1.39]$    & $0.57$ \\
FUB min          & $-1.38$ & $-2.02$ $[-3.09, -1.16]$ & $0.10$ \\
FUB max          & $+3.01$ & $+2.42$ $[+1.28, +4.00]$ & $0.17$ \\
EMP mean         & $+0.00$ & $+0.01$ $[-0.30, +0.32]$ & $0.50$ \\
EMP sd           & $1.00$  & $1.01$ $[0.75, 1.29]$    & $0.53$ \\
EMP min          & $-1.45$ & $-1.99$ $[-3.01, -1.19]$ & $0.09$ \\
EMP max          & $+3.16$ & $+2.74$ $[+1.65, +4.09]$ & $0.23$ \\
\bottomrule
\end{tabular}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\linewidth]{b5_ppc.pdf}
    \caption{Posterior predictive distributions of the four
    test statistics on the FUB (top row) and EMP (bottom row)
    reference series. The red vertical line marks the observed
    value; the Bayesian $p$-value is the probability mass to its
    right. The model is well calibrated on first and second
    moments and slightly short-tailed on the extreme min/max.}
    \label{fig:ppc}
\end{figure}

\section{Tournament robustness: bootstrap confidence intervals}
\label{app:robust}

Section~\ref{sec:application-tournament} reports point
Spearman $\rho$ for every (metric, reference) pair, a Bayesian
posterior credible interval on the IMD-4 vs.\ eco-counter
cell, and a paragraph on bootstrap-CI plus leave-one-out (LOO)
diagnostics. For full transparency,
Figure~\ref{fig:tournament-robust} reproduces the tournament
table with bootstrap CIs printed on every cell. Cells with a
bootstrap CI that strictly excludes zero correspond to
robustly positive correlations; cells whose CI crosses zero
(volumetric and $M$-alone vs.\ FUB perception) are the
fragile baselines. The IMD-4 row has the only set of four
correlations all of which exclude zero.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\linewidth]{b13_tournament_robust.pdf}
    \caption{Same tournament as Figure~\ref{fig:tournament}
    with $1{,}000$-resample bootstrap $95\,\%$ confidence
    intervals on $\rho$ printed in brackets and sub-panel size
    $n$ in each cell. The IMD-4 row is the only one whose four
    CIs all strictly exclude zero; volumetric and $M$-alone vs.\
    FUB perception have CIs spanning zero, confirming that the
    underperformance of these baselines on perception is robust
    to resampling.}
    \label{fig:tournament-robust}
\end{figure}

\section{Future work: a hierarchical Gaussian-process IMD}
\label{app:gp}

The Bayesian formulation of B1 treats every station as having
an independent IMD given the weight posterior. In practice
stations close to one another are likely to share unobserved
environment confounds (a shared cycleway, a shared transit
corridor); a hierarchical model that captures this spatial
correlation would tighten the per-station posterior and
sharpen the within-city heterogeneity diagnostic of
Section~\ref{sec:results-within}.

A natural extension places a Gaussian-process prior on the
station-level deviations from the city mean. For station $s$
in city $i$, write
$\IMD_s = \mu_i + \epsilon_s$ with
\begin{equation}
(\epsilon_{s_1}, \ldots, \epsilon_{s_{n_i}})^{\top}
   \sim
   \mathcal{N}\bigl(\mathbf{0},\, K_i\bigr),
\qquad
K_{i, s_1 s_2}
   = \tau_i^{2}\,
     \exp\!\bigl(-\,d(s_1, s_2)^{2} / \ell^{2}\bigr),
\end{equation}
where $d(s_1, s_2)$ is the geodesic distance between the
stations and $\ell$ is a length scale to be inferred. Hyperpriors
on $\tau_i$ (city-level scale) and $\ell$ (panel-level length)
close the model. Inference would proceed by either an HMC kernel
on the joint $(\mathbf{w}, \boldsymbol{\mu}, \tau, \ell)$ or by
elliptical-slice sampling on the GP nuisance with MH on the
remaining blocks. We expect the per-station credible intervals
to shrink and the within-city heterogeneity diagnostic to
partition cleanly into a station-level GP signal and a
city-level mean signal.

A second extension treats the four components as a multivariate
GP, allowing the components to share spatial structure. The
combined model is a natural next iteration of the indicator and
is left for future work.

% =========================================================================
\bibliography{../references}

\end{document}
